...
sys.path.append(os.path.join(BASE_DIR, 'software/diricore/lib'))
...
def main():
    ...
    for condsampleid, refsampleid in contrasts:
        ...
        for ilevel in xrange(len(shifts)):
            ...
            shifts_file = "%s%s_vs_%s.%s.subsequence_shifts.txt" % (args.outbase, condsampleid, refsampleid, levelname)
            with open(shifts_file, 'w') as sf:
             for codon, shift_value in zip(CODONS, shifts[ilevel]):
              sf.write("{}\t{}\n".format(codon, shift_value))
            print("Shifts saved to {}".format(shifts_file))
